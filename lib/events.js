...
exports.listen = async function (web3, contracts) {
  ...
  _.forEach(contracts.swap721Tokens, (swap721, contractAddr) => {
    async function getTokenDetail(tokenId) {
      ...
      const [terminated, issuer, contractSize, fixLegPayoutPerDay, startTime, endTime, lastSettleTime, margin, totalFixLegPaid, totalFloatingLegPaid, price] = await swap721.getTokenDetail(tokenId);
    ...
    evtStream(swap721.Minted)
      .pipe(cs.object.each(async evt => {
        ...
        // token.price = Math.ceil(token.fixLegPayoutPerDay.toNumber() * ((token.endTime.valueOf() - token.startTime.valueOf()) / 1000) / (3600 * 24));
