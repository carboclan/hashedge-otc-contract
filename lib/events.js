...
exports.listen = async function (web3, contracts) {
  ...
  _.forEach(contracts.swap721Tokens, (swap721, contractAddr) => {
    async function getTokenDetail(tokenId) {
      ...
      const [terminated, issuer, contractSize, fixLegPayoutPerDay, startTime, endTime, lastSettleTime, margin] = await swap721.getTokenDetail(tokenId);

      let status = 0;
      if (terminated) status = -1;
    ...
    evtStream(swap721.Minted)
      .pipe(cs.object.each(async evt => {
        ...
        token.price = token.fixLegPayoutPerDay * (token.endTime.valueOf() - token.startTime.valueOf()) / (3600 * 1000 * 24);
